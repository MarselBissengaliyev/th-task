...
const CreateDocument = (props: Props) => {
  ...
  const handleCreate = async () => {
    try {
      await axios.post("http://localhost:4444/documents/", {
        "outNumber": outNumber,
        ...
        alert('Пост с id: ' + data.data.id  + ' успешно создан')
      })
    } catch (error) {
      alert('Не удалось создать документ: ' + error)
    }
  }
    ...
    <div className="container">
      <nav className='nav-btns'>
        ...
        <div className="nav-btns__right">
          <button className='btn' onClick={handleCreate}>Сохранить</button>
        </div>
      </nav>
